			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return elaina.sendMessage(from,`Limit mu habis bang`, text,{ quoted: mek})
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await elaina.sendMessage(from, {
					...
					elaina.sendMessage(from, `Nih kotnak owner ku `, text, {quoted: hehe})
				}
break
case 'quotesimage':
...
case 'siapakahaku': case 'siapaaku':{
		if (isGame(sender, isPremium, isCreator, isOwner, gcounttuser, glimit)) return sendButMessage(from, lang.limitg(prefix), ` ${ownername}`, [{buttonId: 'limit', buttonText: {displayText: `Cek Limit`, },type: 1,}]);
					...
					if (siapakahaku.hasOwnProperty(sender.split('@')[0])) return reply("⚠️ Masih ada permainan yang sedang berlangsung")
...
case 'leave':
			...
			setTimeout( () => {
			elaina.sendMessage(from, 'Byee', text)
			}, 0)
	 				break
case 'bot':
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
ini_txt += `🌧️ Cuaca : ${get_result.cuaca}\n`
ini_txt += `💨 Angin : ${get_result.angin}\n`
ini_txt += `🗒 ️Description : ${get_result.description}\n`
...
case 'ytmp4': {
			...
			zee.Youtube(mulaikah).then(async (data) => {
                    ...
                    txt += `* Kualitas :* ${data.medias[1].quality}\n`
                    txt += `* Tipe :* ${data.medias[1].extension}\n`
                    txt += `* Ukuran File :* ${data.medias[1].formattedSize}\n`
                    txt += `* Url Source :* ${data.url}\n\n`
                    txt += `_Mohon tunggu sebentar , uploading media..._`
                    sendFileFromUrl(from, data.thumbnail, txt, mek)
