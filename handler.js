    ...
    return new Promise(async(resolve, reject) => {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    this.sendButton(m.chat, `[📮] *Sepertinya Anda Bermain Curang, Menggunakan Calculator*`, author, null, [['Beli Limit', '/buy limit'], ['Menu', '/menu']] , m)
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, `[🪐] *Limit Anda Habis, Beberapa Command Tidak Bisa Di Akses*`, author, null, [['Beli Limit', '/buy limit'], ['Menu', '/menu']] , m)
...
Fitur ini di *disable* !`
    ...
    if (msg) return conn.sendButton(m.chat, danied, msg, `${imgr + 'Accses Danied'}`, [['Menu', '.menu'],['Creator', '.menu']],m)
